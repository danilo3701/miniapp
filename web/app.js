...
/* -------------------------
   10) Отправка данных в бота (Telegram Mini App)
   -------------------------
   Аналогия с ботом:
   - тут мы "отправляем сообщение" обратно в Telegram
   - бот поймает это через F.web_app_data
*/
const sendBtn = $("sendBtn");

if (sendBtn) {
  sendBtn.addEventListener("click", () => {
    // 💬 Собираем данные из state (как будто "FSM данные")
    const payload = {
      clicks: state.clicks,
      mode: state.mode,
    ...
    // 💬 sendData работает только внутри Telegram (в браузере не сработает)
    if (state.isTelegram && window.Telegram && window.Telegram.WebApp) {
      window.Telegram.WebApp.sendData(JSON.stringify(payload));
      log(`✅ Отправлено в бот: clicks=${payload.clicks}`);
    } else {
      log("❌ Это работает только внутри Telegram (не в обычном браузере).");
    }
  });
}
